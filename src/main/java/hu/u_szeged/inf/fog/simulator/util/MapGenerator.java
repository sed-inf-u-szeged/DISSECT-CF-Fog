...
public class MapGenerator {
	...
	public static void mapGenerator(String scriptPath, String resultDirectory, Device... devices) throws IOException {
		MapGenerator.mapGenerator(scriptPath, resultDirectory,  new ArrayList<>(Arrays.asList(devices)));
	}
	public static void mapGenerator(String scriptPath, String resultDirectory, ArrayList<Device> devices) throws IOException {
		...
		for (ComputingAppliance ca : ComputingAppliance.getAllComputingAppliances()) {
			...
			nodeInfoForMapScript = nodeInfoForMapScript.concat(ca.name).concat(",")
					...
					.concat(String.valueOf(ca.geoLocation.longitude)).concat(",")
					.concat(parentName).concat(",")
			...
			for (ComputingAppliance coApp : ca.neighbors) {
				...
				if (checkedAppliances.get(ca) != coApp && checkedAppliances.get(coApp) != ca) {
					checkedAppliances.put(ca, coApp);
					checkedAppliances.put(coApp, ca);
		...
		for(int i = 0; i< devices.size(); i++) {
			FileWriter fw = new FileWriter(resultDirectory+File.separator+"devicePath-" + i + ".csv");
			 ...
			 if(device.mobilityStrategy.getClass().getSimpleName().equals(RandomWalkMobilityStrategy.class.getSimpleName())) {
				 RandomWalkMobilityStrategy ms = (RandomWalkMobilityStrategy) device.mobilityStrategy;
				 fw.write(device.mobilityStrategy.getClass().getSimpleName() + "," + ms.startPosition.latitude + "," + ms.startPosition.longitude + "," + ms.radius +"\n");
			 }else if(device.mobilityStrategy.getClass().getSimpleName().equals(NomadicMobilityStrategy.class.getSimpleName())) {
				 NomadicMobilityStrategy ms = (NomadicMobilityStrategy) device.mobilityStrategy;
				 fw.write(device.mobilityStrategy.getClass().getSimpleName() + "," + ms.startPosition.latitude + "," + ms.startPosition.longitude + "," + 0 +"\n");
			 }else if(device.mobilityStrategy.getClass().getSimpleName().equals(StaticMobilityStrategy.class.getSimpleName())) {
