...
public static void main(String[] args) throws Exception {
    ...
    long tasksize = 150 * 20;
    double instructions = 480; // 1 minute on a1.2xlarge
    String applicationStrategy = "Pliant"; // PliantPredict PushUp HoldDown Random RuntimeAware
...
private static ApplicationStrategy generateAppStrategy(String strategy, double activationRatio, double transferDivider) {
    ...
    }else if(strategy.equals("PliantPredict")) {
        //return new PliantPredictApplicationStrategy(activationRatio, transferDivider);
...
private static void generateDevices(int multiplier) {
    ...
    for(int i = 0; i<2*multiplier; i++) {
        ...
        Repository repo = new Repository(4294967296L, "01device-" + i, 3250, 3250, 3250, latencyMap, stTransitions, nwTransitions); // 26 Mbit/s
        ...
        new SmartDevice(0, 1 * 60 * 60 * 1000, 50, 3, 1 * 60 * 1000, new StaticMobilityStrategy(new GeoLocation(pos[0], pos[1])),
                new RandomDeviceStrategy(), localMachine, 50, false);
    ...
    for(int i = 0; i<6*multiplier; i++) {
        ...
        Repository repo = new Repository(4294967296L, "12device-" + i, 3250, 3250, 3250, latencyMap, stTransitions, nwTransitions); // 26 Mbit/s
        PhysicalMachine localMachine = new PhysicalMachine(1, 0.001, 1073741824L, repo, 0, 0, cpuTransitions);
    ...
    for(int i = 0; i<3*multiplier; i++) {
        ...
        Repository repo = new Repository(4294967296L, "23device-" + i, 3250, 3250, 3250, latencyMap, stTransitions, nwTransitions); // 26 Mbit/s
        PhysicalMachine localMachine = new PhysicalMachine(1, 0.001, 1073741824L, repo, 0, 0, cpuTransitions);
    ...
    for(int i = 0; i<7*multiplier; i++) {
        ...
        Repository repo = new Repository(4294967296L, "34device-" + i, 3250, 3250, 3250, latencyMap, stTransitions, nwTransitions); // 26 Mbit/s
        PhysicalMachine localMachine = new PhysicalMachine(1, 0.001, 1073741824L, repo, 0, 0, cpuTransitions);
    ...
    for(int i = 0; i<2*multiplier; i++) {
        ...
        Repository repo = new Repository(4294967296L, "45device-" + i, 3250, 3250, 3250, latencyMap, stTransitions, nwTransitions); // 26 Mbit/s
        PhysicalMachine localMachine = new PhysicalMachine(1, 0.001, 1073741824L, repo, 0, 0, cpuTransitions);
