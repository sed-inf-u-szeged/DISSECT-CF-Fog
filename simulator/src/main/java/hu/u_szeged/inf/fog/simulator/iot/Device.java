...
public abstract class Device extends Timed {
    ...
    /**
     * This IaasService is a wrapper for the device's communication protocols and their dynamic change.
     */
    public IaaSService communicationProtocolManager;
    ...
    /**
     * The type of data this device produces.
     */
    public TaskType type;

    /**
     * The communication protocols the device can use (WIFI, 5G, LORA).
     */
    public List<String> usableCommunicationProtocols = new ArrayList<>();

    /**
     * The communication protocol repos for the edge device.
     */
    protected List<Repository> communicationProtocolRepos = new ArrayList<>();

    /**
     * The currently used communication protocol, default is LORA (because of the local data processing).
     */
    public String currentCommunicationProtocol = "LORA";

    /**
     * The currently used communication protocol's repository.
     */
    public Repository currentCommunicationProtocolRepo;
    ...
    private void realTransfer() {
        this.currentCommunicationProtocolRepo.contents().stream()
            .filter(storageObject -> !(storageObject instanceof VirtualAppliance)).forEach(storageObject -> {
                ...
                try {
                    ...
                    NetworkNode.initTransfer(storageObject.size, ResourceConsumption.unlimitedProcessing,
                            this.currentCommunicationProtocolRepo, this.caRepository, soe);
                } catch (NetworkException e) {
                    e.printStackTrace();
                }
            }
    ...
    class DeviceDataEvent implements ConsumptionEvent {
        ...
        public void conComplete() {
            currentCommunicationProtocolRepo.deregisterObject(this.so);
            application.computingAppliance.iaas.repositories.get(0).registerObject(this.so);
