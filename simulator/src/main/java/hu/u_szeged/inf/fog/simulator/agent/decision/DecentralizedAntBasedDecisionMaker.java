...
public class DecentralizedAntBasedDecisionMaker extends DecisionMaker {
    ...
    public DecentralizedAntBasedDecisionMaker(ArrayList<StandardResourceAgent> allAgents, int numberOfAnts, int numberOfIterations, double probability, double evaporationRate, int freq, double lambda) {
        ...
        ClusterSorter.lambda = lambda;
    }

    @Override
    public void start(AgentApplication app) {
        //This runs on all StandardResourceAgents, not on the passed list
    ...
    public void generateOffers(AgentApplication app) {
        ...
        for (StandardResourceAgent entry : chosenCluster) {
            agentResourcePairs.addAll(entry.agentStrategy.canFulfill(entry, app.components));
        }
