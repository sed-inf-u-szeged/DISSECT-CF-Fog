...
public class ClusterSorter {
    ...
    static double getScore(StandardResourceAgent node1, StandardResourceAgent node2, AgentApplication application) {
        ...
        //Price
        double node1Price = ResourceAgent.normalize(node1.hourlyPrice, ResourceAgent.minPrice, ResourceAgent.maxPrice);
        double node2Price = ResourceAgent.normalize(node2.hourlyPrice, ResourceAgent.minPrice, ResourceAgent.maxPrice);
        ...
        double quality =
                qBW * application.bandwidthPriority +
                qLat * application.latencyPriority +
                qPrice * application.pricePriority +
                qEnergy * application.energyPriority;


        //Logical distance (penalty: the higher the difference, the higher the penalty, squared ensures high penalty + positive result)
        double distance =
                application.bandwidthPriority * Math.pow(node1BW - node2BW, 2) +
                application.latencyPriority * Math.pow(node1Lat - node2Lat, 2) +
                application.pricePriority * Math.pow(node1Price - node2Price, 2) +
                application.energyPriority * Math.pow(node1Energy - node2Energy, 2);
